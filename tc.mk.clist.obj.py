...
for idt, DTime in enumerate(lDTime):
  ...
  if ((idt==0)or(DTime.month != lDTime[idt-1].month)):
    ...
    # SST
    #-------------------- 
    a2sst = dl.Load_monSfc(model, "sst", Year, Mon)  # t2m is called for d4PDF
    try:
      a2sst = a2sst.filled(miss)
    except:
      pass
    ...
    # Land
    #-------------------- 
    a2land= dl.Load_const(model, "land")
    try:
      a2land = a2land.filled(miss)
    except:
      pass
  ...
  #--------------------
  a2tlw   = dl.Load_6hrPlev(model, "ta",DTime,850)
  a2tmd   = dl.Load_6hrPlev(model, "ta",DTime,500)
  a2tup   = dl.Load_6hrPlev(model, "ta",DTime,plev_up)
  a2ulw   = dl.Load_6hrPlev(model, "ua",DTime,850)
  a2uup   = dl.Load_6hrPlev(model, "ua",DTime,plev_up)
  a2vlw   = dl.Load_6hrPlev(model, "va",DTime,850)
  a2vup   = dl.Load_6hrPlev(model, "va",DTime,plev_up)

  try:
    a2tlw   = a2tlw.filled(miss)
    a2tmd   = a2tmd.filled(miss)
    a2tup   = a2tup.filled(miss)
    a2ulw   = a2ulw.filled(miss)
    a2uup   = a2uup.filled(miss)
    a2vlw   = a2vlw.filled(miss)
    a2vup   = a2vup.filled(miss)
  except:
    pass
