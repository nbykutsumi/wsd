...
for DTime in lDTime:
    ...
    M1 = const["thMt1"]
    M2 = const["thMt2"]
    a2fig = f.mk_tfront(DTime, M1=M1, M2=M2)

    ##-- use binary data --
    #srcDir  = wsbaseDir + "/%s/6hr/front.out/%04d/%02d"%(run,year,mon)
    #srcPath = srcDir + "/front.%04d.%02d.%02d.%02d.%dx%d"%(year,mon,day,hour,ny,nx)
    #a2fig = np.fromfile(srcPath,"float32").reshape(ny,nx)
